...
const LichSuRut = ({ ID }) => {
  ...
  const callDataApi = async () => {
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/admin/users/rut-tien/chi-tiet?id=${ID}`);

    return results.data;
  };
  const getListQuery = useQuery("get-admin-lich-su-rut-tien-user", callDataApi, {
    cacheTime: 0,
  ...
  const GridColDef = [
    ...
    { field: "createdAt", headerName: "Thời gian rút", width: 250 },
  ];
