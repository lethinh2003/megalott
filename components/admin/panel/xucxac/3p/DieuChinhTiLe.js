...
const DieuChinhTiLe = () => {
  ...
  const callDataApi = async () => {
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/admin/games/xucxac3p/ti-le`);

    return results.data;
  };
  const getListQuery = useQuery("get-admin-ti-le-game-xucxac-3p", callDataApi, {
    cacheTime: 0,
  ...
  useEffect(() => {
    if (dataQuery && dataQuery.data) {
      setTiLe({
        tiLeCLTX: dataQuery.data.gameConfigs.xucXacConfigs.xucXac3P.tiLeCLTX,
        tiLe2SoBatKy: dataQuery.data.gameConfigs.xucXacConfigs.xucXac3P.tiLe2SoBatKy,
        tiLe2So: dataQuery.data.gameConfigs.xucXacConfigs.xucXac3P.tiLe2So,
        tiLe3SoBatKy: dataQuery.data.gameConfigs.xucXacConfigs.xucXac3P.tiLe3SoBatKy,
        tiLe3So: dataQuery.data.gameConfigs.xucXacConfigs.xucXac3P.tiLe3So,
      });
    }
  }, [dataQuery]);
  ...
  const handleClickDieuChinh = () => {
    ...
    socket.emit("dieu-chinh-ti-le-xucxac-3p", { tiLe: tiLe }, (callback) => {
      if (callback) {
