...
const ChiTietPhien = ({ ID }) => {
  ...
  const callDataApi = async () => {
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/admin/games/keno3p/chi-tiet?id=${ID}`);

    return results.data;
  };
  const getListQuery = useQuery("get-admin-lich-su-chi-tiet-game-keno-3p", callDataApi, {
    cacheTime: 0,
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.off("refetchDataChiTietPhienGameKeno3P").on("refetchDataChiTietPhienGameKeno3P", ({ phien }) => {
        if (phien == ID) {
          refetch();
        }
      });
      socket.off("timerKeno3P").on("timerKeno3P", (data) => {
        if (phien === data.phien) {
          setTime(data.current_time);
        }
      });
      socket.off("ketQuaDieuChinhKeno3P").on("ketQuaDieuChinhKeno3P", ({ ketQua, phienHienTai }) => {
        if (phien === phienHienTai) {
          setKetQua(ketQua);
        }
      });
      return () => {
        socket.off("refetchDataChiTietPhienGameKeno3P");
        socket.off("timerKeno3P");
        socket.off("ketQuaDieuChinhKeno3P");
      };
    }
  ...
  const handleClickConfirmDieuChinhKetQua = () => {
    ...
    if (socket) {
      socket.emit("set-ket-qua-dieu-chinh-keno-3p", ketQua);
  ...
  return (
    <>
      ...
      <Breadcrumbs aria-label="breadcrumb">
        ...
        <Link underline="hover" color="inherit" href="/admin/games/keno3p">
          Keno3P
        </Link>
        <Typography>Chi tiết</Typography>
      </Breadcrumbs>
      <h1 className="title">Chi Tiết Phiên Keno 3P</h1>
