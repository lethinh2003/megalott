...
const Keno3P = () => {
  ...
  const callDataApi = async () => {
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/admin/games/keno3p`);

    return results.data;
  };
  const getListQuery = useQuery("get-admin-lich-su-game-keno-3p", callDataApi, {
    cacheTime: Infinity,
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.off("refetchDataGameKeno3P").on("refetchDataGameKeno3P", () => {
        refetch();
      });
      return () => {
        socket.off("refetchDataGameKeno3P");
      };
    }
  }, [socket]);
  useEffect(() => {
  ...
  const GridColDef = [
    ...
    { field: "createdAt", headerName: "Thời gian", width: 250 },
    {
      ...
      width: 150,
      getActions: (params) => [
        <Link href={`/admin/games/keno3p/${params.id}`} label="Info">
          <InfoIcon />
        </Link>,
      ],
    },
