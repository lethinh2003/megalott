...
const DieuChinhTiLe = () => {
  ...
  const callDataApi = async () => {
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/admin/games/keno1p/ti-le`);

    return results.data;
  };
  const getListQuery = useQuery("get-admin-ti-le-game-keno-1p", callDataApi, {
    cacheTime: 0,
  ...
  useEffect(() => {
    if (dataQuery && dataQuery.data) {
      setTiLe(dataQuery.data.gameConfigs.kenoConfigs.tiLe1P);
    }
  }, [dataQuery]);
  ...
  const handleClickDieuChinh = () => {
    ...
    socket.emit("dieu-chinh-ti-le-keno-1p", { tiLe: convertNum }, (callback) => {
      if (callback) {
