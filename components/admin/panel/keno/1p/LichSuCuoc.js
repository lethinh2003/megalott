...
const LichSuCuoc = ({ ID }) => {
  ...
  const callDataApi = async () => {
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/admin/games/keno1p/lich-su-cuoc?id=${ID}`);

    return results.data;
  };
  const getListQuery = useQuery("get-admin-lich-su-cuoc-chi-tiet-game-keno-1p", callDataApi, {
    cacheTime: 0,
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.off("refetchDataLichSuCuocGameKeno1P").on("refetchDataLichSuCuocGameKeno1P", ({ phien }) => {
        if (phien == ID) {
          refetch();
        }
      });
      return () => {
        socket.off("refetchDataLichSuCuocGameKeno1P");
      };
    }
  }, [socket]);
  useEffect(() => {
