...
const ChiTietPhien = ({ ID }) => {
  ...
  const callDataApi = async () => {
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/admin/games/keno1p/chi-tiet?id=${ID}`);

    return results.data;
  };
  const getListQuery = useQuery("get-admin-lich-su-chi-tiet-game-keno-1p", callDataApi, {
    cacheTime: 0,
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.off("refetchDataChiTietPhienGameKeno1P").on("refetchDataChiTietPhienGameKeno1P", ({ phien }) => {
        if (phien == ID) {
          refetch();
        }
      });
      socket.off("timerKeno1P").on("timerKeno1P", (data) => {
        if (phien === data.phien) {
          setTime(data.current_time);
        }
      });
      socket.off("ketQuaDieuChinhKeno1P").on("ketQuaDieuChinhKeno1P", ({ ketQua, phienHienTai }) => {
        if (phien === phienHienTai) {
          setKetQua(ketQua);
        }
      });
      return () => {
        socket.off("refetchDataChiTietPhienGameKeno1P");
        socket.off("timerKeno1P");
        socket.off("ketQuaDieuChinhKeno1P");
      };
    }
  ...
  const handleClickConfirmDieuChinhKetQua = () => {
    ...
    if (socket) {
      socket.emit("set-ket-qua-dieu-chinh-keno-1p", ketQua);
