...
const LichSuGame = () => {
  ...
  useEffect(() => {
    if (socket && status === "authenticated") {
      ...
      socket.off("ketQuaPhienHienTaiXucXac5P").on("ketQuaPhienHienTaiXucXac5P", (data) => {
        setListLichSu((state) => {
      ...
      return () => {
        socket.off("ketQuaPhienHienTaiXucXac5P");
      };
    }
  }, [socket, status]);
  ...
  const callDataApi = async (status) => {
    ...
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/games/xucxac5p?results=${itemsPerPage}`);
    return results.data;
  };
  const getListQuery = useQuery(
    ["get-lich-su-game-xucxac-5p", session ? session.user.taiKhoan : null],
    () => callDataApi(status),
    {
      cacheTime: 0,
  ...
  const handleClickLoadmore = async () => {
    try {
      ...
      const results = await axios.get(
        `${process.env.ENDPOINT_SERVER}/api/v1/games/xucxac5p?page=${currentPage}&results=${itemsPerPage}`
      );
