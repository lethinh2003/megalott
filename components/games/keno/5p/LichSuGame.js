...
const LichSuGame = () => {
  ...
  useEffect(() => {
    if (socket && status === "authenticated") {
      ...
      socket.off("ketQuaPhienHienTai5P").on("ketQuaPhienHienTai5P", (data) => {
        setListLichSu((state) => {
      ...
      return () => {
        socket.off("ketQuaPhienHienTai5P");
      };
    }
  }, [socket, status]);
  ...
  const getListQuery = useQuery(
    ["get-lich-su-game-keno-5p", session ? session.user.taiKhoan : null],
    () => callDataApi(status),
    {
      cacheTime: 0,
