...
const LichSuGame = () => {
  ...
  const callDataApi = async (status) => {
    ...
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/games/keno1p?results=${itemsPerPage}`);
    return results.data;
  };
  ...
  const handleClickLoadmore = async () => {
    try {
      ...
      const results = await axios.get(
        `${process.env.ENDPOINT_SERVER}/api/v1/games/keno1p?page=${currentPage}&results=${itemsPerPage}`
      );
