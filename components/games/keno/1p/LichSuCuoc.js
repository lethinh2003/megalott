...
const LichSuCuoc = () => {
  ...
  const callDataApi = async (status) => {
    ...
    const results = await axios.get(
      `${process.env.ENDPOINT_SERVER}/api/v1/games/keno1p/lich-su-cuoc?results=${itemsPerPage}`
    );
    return results.data;
  };
  ...
  const handleClickLoadmore = async () => {
    try {
      ...
      const results = await axios.get(
        `${process.env.ENDPOINT_SERVER}/api/v1/games/keno1p/lich-su-cuoc?page=${currentPage}&results=${itemsPerPage}`
      );
