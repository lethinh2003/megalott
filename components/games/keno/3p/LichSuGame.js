...
const LichSuGame = () => {
  ...
  useEffect(() => {
    if (socket && status === "authenticated") {
      ...
      socket.off("ketQuaPhienHienTai3P").on("ketQuaPhienHienTai3P", (data) => {
        setListLichSu((state) => {
      ...
      return () => {
        socket.off("ketQuaPhienHienTai3P");
      };
    }
  }, [socket, status]);
  ...
  const callDataApi = async (status) => {
    ...
    const results = await axios.get(`${process.env.ENDPOINT_SERVER}/api/v1/games/keno3p?results=${itemsPerPage}`);
    return results.data;
  };
  const getListQuery = useQuery(
    ["get-lich-su-game-keno-3p", session ? session.user.taiKhoan : null],
    () => callDataApi(status),
    {
      cacheTime: 0,
  ...
  const handleClickLoadmore = async () => {
    try {
      ...
      const results = await axios.get(
        `${process.env.ENDPOINT_SERVER}/api/v1/games/keno3p?page=${currentPage}&results=${itemsPerPage}`
      );
