...
const LichSuCuoc = () => {
  ...
  useEffect(() => {
    if (socket && status === "authenticated") {
      ...
      socket.off("updateLichSuCuocCaNhan3P").on("updateLichSuCuocCaNhan3P", (data) => {
        setListLichSu((state) => {
      ...
      return () => {
        socket.off("updateLichSuCuocCaNhan3P");
      };
    }
  }, [socket, status]);
  ...
  const callDataApi = async (status) => {
    ...
    const results = await axios.get(
      `${process.env.ENDPOINT_SERVER}/api/v1/games/keno3p/lich-su-cuoc?results=${itemsPerPage}`
    );
    return results.data;
  };
  const getListQuery = useQuery(
    ["get-lich-su-cuoc-game-keno-3p", session ? session.user.taiKhoan : null],
    () => callDataApi(status),
    {
      cacheTime: 0,
  ...
  const handleClickLoadmore = async () => {
    try {
      ...
      const results = await axios.get(
        `${process.env.ENDPOINT_SERVER}/api/v1/games/keno3p/lich-su-cuoc?page=${currentPage}&results=${itemsPerPage}`
      );
