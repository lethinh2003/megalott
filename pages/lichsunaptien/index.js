...
import DanhSachLichSu from "../../components/lichSuNapTien/DanhSachLichSu";
const Home = () => {
  ...
  const callDataApi = async (status, pageParam) => {
    ...
    const results = await axios.get(
      `${process.env.ENDPOINT_SERVER}/api/v1/lichsunap?page=${pageParam}&results=${itemsPerPage}`
    );
    return results.data;
  };
  const getListQuery = useInfiniteQuery(
    ["get-lich-su-nap-tien", session ? session.user.taiKhoan : null],
